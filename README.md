...
- **流式回复**：支持 SSE 流式输出，前端可做打字机效果，无需等待整段生成完毕
...
│   └── extract_portraits_from_swf.py  # 从 SWF 导成立绘
├── services/               # 业务逻辑层
...
- **后端**：FastAPI + Uvicorn
- **流式接口**：Ask 接口支持 `?stream=true` 的 SSE 流式响应，前端对接可参考 [docs/STREAMING_API_FRONTEND.md](docs/STREAMING_API_FRONTEND.md)
- **RAG 框架**：LlamaIndex
