...
| 2.9    | 编写单元测试：`prepare_task_context` 各类型返回值正确性、`update_task_draft` 增量修改、校验管线全覆盖（合法/非法用例）、奖励计算边界测试 | `tests/test_agent_tools/`        | 2.2-2.8  |
...
| 4.6    | 端到端集成测试：模拟完整的 "对话 → `prepare_task_context` → `draft_agent_task` → 讨价还价（`update_task_draft`）→ 接受 → 写入" 流程，以及 "修改要求 → 局部修改" 和 "更改类型 → 重新走全流程" 两种协商路径 | `tests/test_task_generation/`    | 4.1-4.5  |
