...
| `reward_item_candidates` | `list` | 根据 `reward_types` 筛选的可选奖励物品 `[{name, type, price, level?, source}]`，`source` 为 `"NPC商店"` 或 `"任务奖励可选"`（任务侧按主线/前置/佣兵/其余分档，层内随机后截断）；可选 `reward_keywords` 调整优先级；`武器` 类奖励含 `use=手雷` 的消耗品 |
