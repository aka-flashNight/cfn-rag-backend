...
- **通用规则**
  - 以下目录均为项目外部游戏目录 `resources\data` 内的子文件夹目录，用于查询游戏各项数据。
...
- 特点：
  ...
  - 部分委托会扩展副本难度（如增加 `修罗` / `地狱`），但这是 mercenary 自己的逻辑，Agent 任务只允许要求普通。
...
- Agent 在构造**收集类任务**时，可以考虑：
  ...
  - 将**本NPC商店**的物品作为奖励（物品如果有等级需求，则需要符合或小于玩家对应的等级区间）。
...
1. **玩家与 NPC 对话（LLM 对话层）任务类型与奖励计算**
   - 对话中，Agent 可以根据上下文判断是否合适提出新任务：
     ...
     - 资源收集类（基础材料/药剂/食品收集一定数量并提交，奖励额外增加提交品价值的 1.5~2 倍，提交品的总价值不能超过基础奖励上限的200%）；
     - 特殊物品收集类（装备/插件/贵重消耗品收集一个并提交，奖励额外增加提交品价值的 1.5~2 倍，提交品的总价值不能超过基础奖励上限的300%）；
     ...
     - 装备获取类委托（购买或合成获得特定装备/材料，奖励额外增加该物品价值的1.5~2倍。）；
   - 除了任务要求可以划分类别外，任务奖励也可以根据NPC设定与对话剧情来划分不同类别：
     - 金币（最常规的奖励）；
     - 经验（仅挑战类可以给大量经验，其他类型给少量经验）；
     - 药剂（常见奖励）；
     - 弹药（可选奖励）；
     - K点（阶段4及以上时可选奖励）；
     - 技能点（可选奖励）；
...
3. **任务协商与生命周期（跨多轮对话）**
   ...
   **3.1 任务发起**
   ...
   - 决策阶段通过工具调用查询：玩家进度信息 → 可用关卡与掉落 → 物品价格 → NPC 商店 → 现有任务（避免重复）；
   - LLM 调用 `draft_agent_task` 工具，输出结构化的**任务草案（Task Draft）**（详见 6.4 结构化输出）；
   ...
   **3.2 协商与调整**
   ...
   - 在后续对话中，玩家可以：
     ...
     - **讨价还价**（*"奖励太少了"* / *"能多给点金币吗"*）→ LLM 在允许范围内调整奖励后重新提出：
       - 好感度 ≥ 50 → 可在基础奖励上浮动 +10%～+50%；
       - 50 > 好感度 >= 20 → 可在基础奖励上浮动 +1%～+20%；
       - 好感度 < 20 → 几乎不让步甚至可以减少奖励；
       - 每次调整后更新 `session_task_drafts` 中的草案；
     - **修改要求**（*"换个简单点的"* / *"我不想打那个关"*）→ LLM 重新调用 `draft_agent_task` 生成新草案；
...
class TaskDraft(TypedDict):
    """待确认的任务草案"""
    task_type: str                    # 任务类型：问候/通关/挑战/资源收集/特殊收集/持有/通关收集/通关持有/装备获取
...
所有工具使用 OpenAI Function Calling 格式定义（兼容 LangChain `@tool` 装饰器）。工具分为**只读查询工具**和**写入工具**两类。

**A. 只读查询工具**

| 工具名                           | 参数                                    | 返回值                                         | 说明                                     |
|----------------------------------|----------------------------------------|------------------------------------------------|------------------------------------------|
| `get_player_progress`            | 无（从 state 读取）                     | `{stage, stage_name, level_range, main_task_range}` | 当前阶段配置                             |
| `get_npc_info`                   | 无（从 state 读取）                     | `{npc_name, affinity, relationship, faction, has_shop, shop_items?}` | 当前 NPC 信息 + 商店                     |
| `query_items`                    | `keyword?: str, type?: str, max_level?: int` | `list[{name, type, use, price, level?}]`（上限 20 条） | 模糊搜索物品                             |
| `query_candidate_stages`         | `stage?: int`                           | `list[{area, name, unlock_id, difficulties}]`  | 符合进度的可用关卡列表                    |
| `query_stage_loot`               | `area: str, stage_name: str`            | `list[{item_name, min_qty, max_qty, crate_type}]` | 指定关卡的箱子掉落                       |
| `query_existing_tasks`           | `npc_name?: str, task_type?: str`       | `list[{id, title, type, npc}]`（上限 15 条）   | 查询已有任务（避免重复）                  |
| `calculate_reward_budget`        | `task_type: str, stage: int`            | `{base_min, base_max, multiplier, final_min, final_max}` | 根据类型和阶段计算奖励预算区间            |
...
| 工具名                           | 参数                                    | 返回值                                         | 说明                                     |
|----------------------------------|----------------------------------------|------------------------------------------------|------------------------------------------|
| `draft_agent_task`               | `TaskDraft`（完整的结构化任务草案）       | `{success, draft_id, validation_errors?}`      | 生成并校验任务草案，暂存到 DB             |
...
| 工具名                           | 参数                                    | 返回值                                         | 说明                                     |
|----------------------------------|----------------------------------------|------------------------------------------------|------------------------------------------|
| `update_npc_mood`                | `favorability_change: int, emotion: str` | 无需返回
                                       | 沿用现有定义                        |
...
```json
{
  ...
  "properties": {
    "task_type": {
      "type": "string",
      "enum": ["问候", "通关", "挑战", "资源收集", "特殊收集", "持有", "通关收集", "通关持有", "装备获取"]
    },
...
当 `draft_agent_task` 或 `confirm_agent_task` 被调用时，后端按以下顺序逐项校验，全部通过才允许暂存/写入：
...
Layer 2 — NPC 层（按 NPC 缓存，同一 NPC 的多次请求相同）:
  ├── NPC 名、性别、阵营、称号
  ├── NPC 可用情绪标签
  └── NPC 人格约束（见 6.6：该 NPC 擅长/不擅长发布什么类型的任务）

Layer 3 — 会话层（按请求变化，但变化量小）:
  ├── 当前好感度与关系等级
  ├── 玩家进度阶段
  ├── 玩家身份描述
  └── 待确认任务草案摘要（如有）
...
Layer 1 + Layer 2 构成了最稳定的前缀，跨请求变化极小，缓存命中率最高。Layer 3 变量通过插值注入，token 变化量很小（几十个 token），仍可命中大部分缓存。Layer 4 是最大的变化量来源，但排在最后，不影响前缀。
...
| 普通成员 NPC                               | 问候、资源收集（低级）、持有（低级）   | 金币、药剂、食品、弹药               | "帮我带点药品回来"             |
...
| 1.8    | 编写单元测试：至少覆盖物品查询、关卡筛选、奖励价值计算                              | `tests/test_game_data/`          | 1.3-1.7  |

**验收标准**：`GameDataRegistry` 在服务启动时自动加载全部数据，所有查询方法可在 < 1ms 内返回结果。

#### Phase 2：Agent 工具定义与校验逻辑

**目标**：定义全部 LLM 工具 schema，实现工具执行逻辑和校验管线。

| 步骤   | 具体任务                                                                         | 产出文件                          | 依赖     |
|--------|---------------------------------------------------------------------------------|----------------------------------|----------|
| 2.1    | 定义所有只读查询工具的 OpenAI Function schema（JSON）                              | `services/agent_tools/schemas.py` | Phase 1  |
| 2.2    | 实现只读查询工具的执行器：将 tool_call 参数 → `GameDataRegistry` 查询 → JSON 返回  | `services/agent_tools/query_tools.py` | 2.1      |
| 2.3    | 定义 `draft_agent_task` 的 JSON Schema（6.4.1）                                  | `services/agent_tools/schemas.py` | Phase 1  |
| 2.4    | 实现校验管线（Validation Pipeline，V1-V10）                                       | `services/agent_tools/validator.py` | Phase 1  |
...
| 2.6    | 实现写入工具执行器（draft / confirm / cancel）                                    | `services/agent_tools/task_tools.py` | 2.4, 2.5 |
| 2.7    | 实现 `session_task_drafts` 数据库表（在 `MemoryManager` 中扩展或独立模块）          | `services/task_draft_store.py`   | 无       |
| 2.8    | 实现 `calculate_reward_budget` 工具（奖励预算计算逻辑，含类型倍率 + 好感修正）       | `services/agent_tools/reward_calculator.py` | Phase 1 |
| 2.9    | 编写单元测试：校验管线全覆盖（合法/非法用例）、奖励计算边界测试                      | `tests/test_agent_tools/`        | 2.2-2.8  |

**验收标准**：所有工具可独立调用并返回正确结果；校验管线能拦截所有已知的非法输入。
...
| 3.12   | Prompt 模板重构：按 6.5.3 的分层结构重组 system prompt                             | `services/agent_graph/prompts.py`| 3.3      |
...
| 步骤   | 具体任务                                                                         | 产出文件                          | 依赖     |
|--------|---------------------------------------------------------------------------------|----------------------------------|----------|
| 4.1    | 完善 `draft_agent_task` 工具的 LLM 提示词：在 system prompt 中注入任务发布规则（6.6） | `services/agent_graph/prompts.py`| Phase 3  |
| 4.2    | 实现任务草案的 DB 持久化（`session_task_drafts` 表的 CRUD 操作）                   | `services/task_draft_store.py`   | 2.7      |
| 4.3    | 实现协商状态管理：在 `prepare_context` 节点中加载待确认草案并注入 prompt            | `services/agent_graph/nodes.py`  | 4.2      |
...
| 4.6    | 端到端集成测试：模拟完整的 "对话→发起任务→讨价还价→接受→写入" 流程                  | `tests/test_task_generation/`    | 4.1-4.5  |
