...
- `tasks2.json` 及其它 `*_tasks.json`：**支线/系统任务**，包括：
  ...
  - `challenge_tasks.json`：挑战类任务（偏高难度、经验值奖励为主）；
...
- **任务奖励价值量化思路**
  ...
  - 不同主线阶段设定不同的**奖励价值区间**，并根据：
    ...
    - 当前 NPC 好感度；
    - 玩家在对话中是否讨价还价；
    - 任务类型（问候 / 通关 / 挑战 / 收集 / 装备缴纳委托等）；
    - 基础奖励区间计算公式：[阶段 * 1万金币, 阶段 * 2万金币] 的上下限区间。
    - 有通关需求的情况，奖励额度 * 2 。有收集需求的话，奖励允许区间下限按收集品价值额外 +1 倍、上限额外 +2 倍（在 capped 后计入，上限不超过当前任务奖励额度的 200%，装备类不超过 300% ）。持有类需求，则额外增加物品的 0.5 倍价值（上限不超过当前任务奖励额度的 50% ）；
    - 来在区间内偏多或偏少地选择实际奖励。
...
- Agent 一般**不会直接使用 K 点商城**物品作为任务奖励（避免破坏经济系统），但可以用作为**任务物品要求**（物品如果有等级需求，则需要符合或小于玩家对应的等级区间）并给出匹配价值的奖励。（只有在非常合适的情景下才可以发布此任务，例如稀有武器/装备缴纳的委托，不作为常规任务类型）
...
- **与 Agent 任务的关系（建议用法）**
  - 当设计“改造/强化相关”的任务时，可以：
    ...
    - 也可以作为任务奖励，但要根据玩家的进度来匹配。阶段1-7之中，低级对应1-2，中级对应2-3，高级对应4及以上。
...
1. **玩家与 NPC 对话（LLM 对话层）任务类型与奖励计算**
   - 对话中，Agent 可以根据上下文判断是否合适提出新任务：
     - 问候类/传话类/闲聊类（仅基础奖励）；
     - 通关类（低难度地图，基础奖励翻倍，但经验值少）；
     - 挑战类（高难度，基础奖励翻倍，且经验值占比较多）；
     - 资源收集类（基础材料/药剂/食材收集一定数量并提交，奖励允许区间对提交品：下限 +1× 价值、上限 +2× 价值，提交品的总价值不能超过基础奖励上限的200%）；
     - 特殊物品获取类（装备/插件/食品/合成消耗品收集一个并提交，同上 1×/2× 规则，提交品的总价值不能超过基础奖励上限的300%）；
     - 持有类（物品/情报收集但无需提交，奖励额外增加持有品价值的0.5倍，但额外增加的奖励上限不能超过基础奖励上限的50%）；
     - 通关 + 收集类（基础奖励翻倍，且叠加提交品价值的 1×（下限）~2×（上限）。必须是通关对应关卡可以从中获取到所有提交品。）；
     - 通关 + 持有类（基础奖励翻倍，且叠加持有品价值的0.5倍。必须是通关对应关卡可以从中获取到所有持有品。）；
   - 除了任务要求可以划分类别外，任务奖励也可以根据NPC设定与对话剧情来划分不同类别：
     - 金币（最常规的奖励）；
     - 经验值（常规奖励，但仅挑战类可以给大量经验值，其他类型给少量经验值）；
     - 药剂（可选奖励）；
     - 弹夹（可选奖励）；
     - K点（阶段4及以上时可选奖励）；
     - 技能点（可选奖励）；
     ...
     - 插件（可选奖励，仅限当前NPC的商店）；
   - 基础奖励区间计算公式：[阶段 * 1万金币, 阶段 * 2万金币] 的上下限区间，阶段为1-7的整数。
   - 玩家可以选择接受/拒绝/讨价还价。
...
class TaskDraft(TypedDict):
    """待确认的任务草案"""
    task_type: str                    # 任务类型：["问候", "传话", "通关", "清理", "挑战", "切磋", "资源收集", "装备缴纳", "特殊物品获取", "物品持有", "通关并收集", "通关并持有"] 
...
```
Step 1: LLM 调用 prepare_task_context(task_type, reward_types [, requirement_keywords?, reward_keywords?])
...
| 工具名 | 参数 | 返回值 | 说明 |
|--------|------|--------|------|
| `prepare_task_context` | `task_type`, `reward_types`；可选 `requirement_keywords: string[]`、`reward_keywords: string[]` | `TaskContext`（详见下文） | 第一步：意向任务类型与奖励偏好；可选关键词为并集匹配，用于需求侧/奖励侧候选项分层（层内随机）；多来源剩余名额：两档 8:2、三档 5:3:2（四舍五入），不足则从高档向低档补齐 |
...
- `regular`：常规奖励，可多选。金币为最常规奖励；经验值为常规奖励，但仅挑战类可给大量经验值，其他类型给少量经验值。
- `optional`：可选奖励，可多选。其中 K 点仅阶段 4 及以上时可选；武器/防具/插件仅限当前 NPC 商店有售时可选，且武器/防具符合玩家等级区间。
- 后端根据 LLM 选择的奖励类型，返回对应的可选奖励物品列表。

**可选关键词（`requirement_keywords` / `reward_keywords`）：**

- `requirement_keywords`：作用于各任务类型的**需求侧**列表（如关卡名/区域说明、提交或持有物品、NPC 名与称号等），优先级一般为：**关键词命中 → 符合当前进度 → 低于进度**（具体层数因类型而异）；**同一优先级桶内随机**，避免长尾候选项永远进不了截断列表。
- `reward_keywords`：仅作用于通用字段 `reward_item_candidates`。命中项优先，并按 **NPC 商店 → 任务奖励进度档 1→2→3→4** 分层（层内随机）；若传了关键词且命中项未占满上限，**剩余名额**在未命中项中按 **NPC 商店 : 任务奖励池 ≈ 8 : 2**（低档约 `round(R×20%)`）分配，不足从高档池依次补齐。
- 特殊物品获取 / 装备缴纳等在关键词命中项之后，对**剩余名额**按来源 **合成 : 非本阵营商店 : K 点 = 5 : 3 : 2** 分配（同上，池内随机后按比例取，不足从高到低补）。
...
| `reward_item_candidates` | `list` | 根据 `reward_types` 筛选的可选奖励物品 `[{name, type, price, level?, source}]`，`source` 为 `"本NPC商店"` 或 `"任务奖励可选"`（任务侧按主线/前置/佣兵/其余分档，层内随机后截断）；可选 `reward_keywords` 调整优先级；`武器` 类奖励含 `use=手雷` 的消耗品 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `special_items` | `list` | 可提交的特殊物品列表 `[{name, type, price, source, plugin_tier?}]`，通常只需要 1 个物品。非装备（武器/防具已排除），含三来源：`"合成"`（配方产出）、`"非本阵营商店"`（其他阵营 NPC 商店）、`"K点商店"`（K 价 ≤ `玩家进度 stage × 100`）。**去重**：同名只保留来源优先级更高的一条，顺序为 **合成 ＞ 非本阵营商店 ＞ K 点**。**单价门槛**（仅本列表）：`source == "合成"` 时要求 `price ≥ 1000`；`"非本阵营商店"` / `"K点商店"` 要求 `price ≥ int(reward_budget.final_min × 10%)`（`final_min` 缺失或为 0 时该分支门槛为 0）。**排序与条数**：按上述来源优先级，同来源内按 `price` 降序，**最多 25 条**。已按玩家等级上限筛除 `level` 超限物品。 |
...
- 列表类返回做**数量截断**（多数列表约 20 条；**特殊物品获取**的 `special_items` 至多 **25** 条），并可在部分字段超出上限时附带 `"truncated": true` 标记，避免 token 超限；
...
收集/持有加成（叠加到乘数后的基础上）:
  finish_submit_items:  额外下限 += 提交品总价值 × 1.0，额外上限 += 提交品总价值 × 2.0（均基于 capped 后的提交品价值），提交品总价值上限 = 基础奖励 × 200%
...
|-------------------------------------------|--------------------------------------|--------------------------------------|-------------------------------|
| 高级军事 NPC（如 Andy Law、将军等）          | 通关、挑战、通关+收集                 | 金币、经验值、武器、强化石              | "去商业区清理一下丧尸"         |
| 商人 NPC（有商店的 NPC）                    | 资源收集、装备缴纳、持有              | 金币、商店物品、材料                  | "帮我收集一些材料"             |
| 普通成员 NPC                               | 问候、资源收集（低级）、持有（低级）   | 金币、药剂、食品、弹夹               | "帮我带点药品回来"             |
| 科技/学术 NPC（大学等）                     | 收集（特殊材料/插件）、装备缴纳       | K 点、技能点、插件、合成材料          | "帮我找一些研究材料"           |
...
- 无商店：*"你不经营商店，奖励以金币、经验值、药剂等通用物资为主。"*
...
| D9 | 挑战类任务的奖励主要是经验值？                              | 是，挑战类（修罗/地狱难度）奖励中经验值占比应 ≥ 50%，金币占比可降低      | 高难度战斗回报应偏向角色成长而非经济收益，平衡游戏经济 |
