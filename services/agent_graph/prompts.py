...
■ 两步式调用流程：
  ...
  Step 2: 根据返回数据调用 draft_agent_task(TaskDraft)
    - 以 JSON 结构化参数输出任务草案（标题、描述、前置任务、通关要求、
      提交/持有物品、奖励、接取/完成对话文本）。
    - 后端会校验草案合法性，校验通过则暂存等待玩家确认；
      校验失败会返回具体错误，你需要根据错误调整后重新提交。

■ 任务类型简要说明：
  - 问候/传话：无战斗要求，仅需对话，基础奖励最低。
  - 通关/清理：要求通关指定关卡（普通/冒险难度），基础奖励×2。
  - 挑战：要求高难度通关（修罗/地狱），基础奖励×2，经验占比≥50%。
  - 切磋：要求通关当前NPC的切磋副本（需NPC有此能力）。
  - 资源收集：收集食材/药剂/材料/弹夹并提交，奖励额外+提交品价值的1.5~2倍。
  - 装备获取/特殊物品获取：获取装备或特殊物品并提交，奖励额外+提交品价值的1.5~2倍。
...
■ 协商流程（跨多轮对话）：
  ...
  - 玩家可以接受、拒绝、讨价还价或要求修改：
    ...
    · 讨价还价 → 调用 update_task_draft 在允许范围内调整奖励（最多2次），
...
def build_layer2(
    *,
    ...
    return (
        ...
        "  · 商人NPC（有商店）→ 资源收集、装备获取、物品持有，奖励偏向金币/商店物品/材料。\n"
        "  · 普通成员NPC → 问候、低级资源收集、低级物品持有，奖励偏向金币/药剂/食品/弹夹。\n"
        "  · 科技/学术NPC → 收集特殊材料/插件、装备获取，奖励偏向K点/技能点/插件/合成材料。\n"
    )
