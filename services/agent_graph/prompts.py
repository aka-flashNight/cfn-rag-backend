...
4. 动作与台词格式：非必要时不出现动作描写。若需表达肢体动作、神态或环境描写，\
必须且只能使用全角粗括号【】包裹；台词直接输出，不加引号；\
严禁用半角括号 () 或星号 * 描述动作。\
你输出的动作若涉及人称，一律单独起一行，采用第三人称视角：用角色名指代你自己，用「你」指代玩家。
5.全角粗括号【】仅可用于动作/神态/环境描写。除了动作/神态/环境描写以外，任何内容都不得填入全角粗括号【】之中。
6.若输出对话时，发现此前工具调用失败，要以执行失败为前提继续对话。例如任务发布工具失败时，取消/拒绝发布任务，而不是继续发布。
"""
...
def build_layer2(
    *,
    ...
    return (
        ...
        "  · 高级NPC → 传话(外交)、通关、清理、挑战、通关并收集、通关并持有等，奖励偏向金币/经验值/武器/强化石/技能点等。\n"
        "  · 商人NPC（有商店）→ 传话(商业联络)、装备缴纳、特殊物品获取、物品持有(合成物品查看效果)、资源收集等，奖励偏向金币/商店物品/材料等。\n"
        "  · 普通成员NPC → 问候、传话、低级资源收集、低级物品持有等，奖励偏向金币/药剂/食品/弹夹等。\n"
        "  · 科技/学术NPC → 传话(学术交流)、特殊物品获取(高级材料/插件)、物品持有(情报)、装备缴纳等，奖励偏向金币/K点/技能点/插件/合成材料/强化石/战宠灵石等。\n"
    )
