...
async def call_llm_stream(
    *,
    ...
    在流式 tool_calls 聚合过程中，若某 index 上 ``update_npc_mood`` 的参数 JSON 已完整可解析，
    则额外 yield ("mood_update", {"emotion", "favorability_change"})（每 index 至多一次）；
    ...
    def _emit_mood_updates():
        """同一 tool_call index 在参数 JSON 可解析且为 update_npc_mood 时，最多 yield 一次 mood_update。"""
        for idx in sorted(tool_calls_acc.keys()):
            if idx in mood_update_emitted:
                continue
            acc = tool_calls_acc[idx]
            name = (acc.get("name") or "").strip()
            if name != "update_npc_mood":
                continue
            raw = (acc.get("arguments") or "").strip()
            if not raw:
                continue
            try:
                obj = json.loads(raw)
            ...
            if "emotion" not in obj or "favorability_change" not in obj:
                continue
            mood_update_emitted.add(idx)
            yield (
                "mood_update",
                {
                    "emotion": str(obj.get("emotion", "")),
                    "favorability_change": int(obj.get("favorability_change", 0)),
                },
            )
